...
@Singleton
public class SurveyDSController extends AbstractDSController {
	...
	public Result view(Request request, Long id) {
		...
		Map<Long, String> tokens = new java.util.HashMap<>();
		for (Participant p : participants) {
			tokens.put(p.getId(), tokenResolverUtil.getParticipationToken(ds.getProject().getId(), p.getId()));
		}

		return ok(views.html.datasets.survey.view.render(ds, username, participants, tokens, request));
	}
	...
	public Result recordForm(Request request, Long id, String invite_token) {
		...
		if (!project.hasParticipant(participant)) {
			return redirect(HOME).addingToSession(request, "error", "Participant does not belong to the project.");
		}
		...
		if (template_text == null) {
			// no form to render
			return ok(views.html.datasets.survey.notavailable.render(ds));
		}
		...
		return ok(views.html.datasets.survey.record.render(csrfToken(request), invite_token, ds));
	}
	...
	public Result record(Request request, Long id, String invite_token) {
		...
		final SurveyDS fmsc = (SurveyDS) datasetConnector.getDatasetDS(ds);
	...
	public Result visualize(Request request, Long id) {
		...
		return ok(views.html.datasets.survey.visualize.render(ds, visualization));
	}
	...
	private Source<ByteString, ?> internalExportRaw(Dataset ds, long limit, long start, long end) {
		SurveyDS fmsc = (SurveyDS) datasetConnector.getDatasetDS(ds);
		return createStream().mapMaterializedValue(sourceActor -> {
			CompletableFuture.runAsync(() -> fmsc.export(sourceActor, Collections.emptyList(), limit, start, end));
			return sourceActor;
		});
	}
	...
	public Result duplicate(Request request, Long id) {
		...
		return ok(views.html.datasets.survey.duplicate.render(csrfToken(request), ds));
	}
}
