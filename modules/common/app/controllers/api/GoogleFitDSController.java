...
public class GoogleFitDSController extends AbstractDSController {

	private final Configurator configurator;

	@Inject
	public GoogleFitDSController(Configurator configurator, FormFactory formFactory, SyncCacheApi cache,
	...
	public Result view(Request request, Long id) {
		...
		Optional<OnboardingMessage> msg = onboardingSupport.setFlashMsg(ds.getProject(), username, "googlefit_ds");
		return ok(views.html.datasets.googlefit.view.render(ds, username, msg, request));
	}
	...
	public Result add(Request request, Long id) {
		...
		return ok(views.html.datasets.googlefit.add.render(csrfToken(request), p));
	}
	...
	public Result addMe(Request request, Long id) {
		...
		Dataset ds = datasetConnector.create(df.get("dataset_name"), DatasetType.GOOGLEFIT, p, df.get("description"),
		        df.get("scopes_main"), df.get("isPublic"), df.get("license"));
	...
	public Result edit(Request request, Long id) {
		...
		return ok(views.html.datasets.googlefit.edit.render(csrfToken(request), ds));
	}
	...
	public Result editMe(Request request, Long id) {
		...
		// FIXME Eden: can you revise this implementation to make use of existing functionality such as
		// Dataset.isActive() ?
		// DatasetUtils.checkStatus(oldDates, dates, ds.id);
	...
	private void updateDates(Dataset ds, DynamicForm df) {
		...
		// check and update wearables if necessary after changing the dates of GoogleFit dataset
		if (ds.getStart() != null) {
	...
	private Source<ByteString, ?> internalExportRaw(Dataset ds, List<Long> wearablesIds, long limit, long start,
		...
		return createStream().mapMaterializedValue(sourceActor -> {
			CompletableFuture.runAsync(() -> gfds.export(sourceActor, limit, start, end));
			return sourceActor;
		});
	}
